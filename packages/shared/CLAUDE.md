...
4. **Most DB timestamps are milliseconds** (`Clock.System.now().toEpochMilliseconds()`). Exceptions: `watch_history.position_seconds` / `duration_seconds` (media offsets, not wall-clock) and **`epg_programmes.start_time` / `end_time` — XMLTV epoch SECONDS, not ms** (matches xmltv.dtd; `XmltvParser` emits `epochSeconds`, guide + catch-up compare against `clock()/1000`; do NOT flip to ms — MB-390). Document the unit on every new timestamp column: `-- ms since epoch` (or `-- epoch seconds`).
