...
class CatchupServiceTest {
    ...
    @Test fun resolvesXtreamCatchupFromLiveChannel() =
        runTest {
    ...
    @Test fun rejectsFutureProgramme() =
        runTest {
    ...
    @Test fun rejectsWhenNoMatchingChannel() =
        runTest {
            val (_, contentRepo, sourceRepo) = bootstrap()
            val svc = CatchupService(contentRepo, sourceRepo, clock = { (apr15_1430 + oneHour) * 1000L })
            val result =
    ...
    @Test fun resolvesM3uCatchupViaShiftMetadata() =
        runTest {
            val (db, contentRepo, sourceRepo) = bootstrap()
            val source =
                sourceRepo.addSource(
    ...
    @Test fun rejectsOutsideArchiveWindow() =
        runTest {
            val (db, contentRepo, sourceRepo) = bootstrap()
            val source =
                sourceRepo.addSource(
                ...
                http =
                    object : com.yancotv.shared.http.HttpClient {
                        override suspend fun getJson(
                            url: String,
                            options: com.yancotv.shared.http.HttpRequestOptions,
                        ): Any? = null
