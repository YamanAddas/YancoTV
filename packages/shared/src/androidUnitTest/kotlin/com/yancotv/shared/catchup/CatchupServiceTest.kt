...
class CatchupServiceTest {
    ...
    @Test fun resolvesXtreamCatchupFromLiveChannel() = runTest {
        ...
        val result =
            svc.resolve(
                programme(channel = "cnn.us", start = apr15At1430, end = apr15At1430 + oneHour),
            )
    ...
    @Test fun rejectsFutureProgramme() = runTest {
        ...
        val result =
            svc.resolve(
                programme(channel = "cnn.us", start = apr15At1430 + oneHour, end = apr15At1430 + 2 * oneHour),
            )
    ...
    @Test fun rejectsWhenNoMatchingChannel() = runTest {
        ...
        val result =
            svc.resolve(
                programme(channel = "unknown.tv", start = apr15At1430, end = apr15At1430 + oneHour),
            )
        val reason = assertIs<CatchupService.Resolution.Unavailable>(result).reason
    ...
    @Test fun resolvesM3uCatchupViaShiftMetadata() = runTest {
        ...
        val now = apr15At1430 + 2 * oneHour
        val svc = CatchupService(contentRepo, sourceRepo, clock = { now * 1000L })
        val result =
            svc.resolve(
                programme(channel = "espn.us", start = apr15At1430, end = apr15At1430 + oneHour),
            )
        val playable = assertIs<CatchupService.Resolution.Playable>(result)
    ...
    @Test fun rejectsOutsideArchiveWindow() = runTest {
        ...
        val tenDaysAgo = apr15At1430 - 10 * 86_400L
        val svc = CatchupService(contentRepo, sourceRepo, clock = { apr15At1430 * 1000L })
        val result =
            svc.resolve(
    ...
    private data class Bootstrap(val db: YancoDb, val contentRepo: ContentRepository, val sourceRepo: SourceRepository)
    ...
    private fun bootstrap(): Bootstrap {
        ...
        val sourceRepo =
            SourceRepository(
                ...
                http =
                object : com.yancotv.shared.http.HttpClient {
                ...
                fileReader =
                object : com.yancotv.shared.sources.FileContentReader {
