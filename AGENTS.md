...
2. **One ExoPlayer, one mpv.** Android shares a single `ExoPlayer` between mini-preview and fullscreen by swapping the output Surface on that one player (`setVideoSurface` / `clearVideoSurface`, symmetric both ways — *not* `PlayerView.switchTargetView()`, which the code doesn't use). The planned cast feature (MK.26) preserves this single-player rule: Track B's Media3 `CastPlayer` wraps the same ExoPlayer, and Track A's LAN handoff drives a *remote* YancoTV's own single player. Desktop goes through `IPlayer` (`src/main/player/player.interface.ts`) — never call mpv directly.
...
- **What this does NOT expose:** Sentry telemetry (HTTPS-only by SDK config). Coil image fetches go through a client that is deliberately *exempt* from the cleartext allow-list (carve-out below) so plain-`http://` channel logos load — those images carry no credentials, so the exposure is cosmetic (an on-path attacker could swap a logo image, nothing more). Adding any *credential-bearing* `http://` traffic outside user-configured provider hosts would expose real data — flag it in review.
- **Defense-in-depth (shipped, `MK.SEC.A/B/C`):** an OkHttp-application-layer allow-list seeded from the `sources` table refuses HTTP requests to hosts that aren't in the user's source list — without flipping the manifest setting (which would break legitimate HTTP providers at the OS layer). **Carve-out (2026-05-20):** Coil's image loader strips the interceptor in `YancoApp.onCreate` so channel logos aren't blanked by it. Tracked under [bugs.md MB-203](bugs.md); accepted-risk detail in [docs/security/AUDIT_NOTES.md](docs/security/AUDIT_NOTES.md).
