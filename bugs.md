...
| MB-25 | Mobile | Critical | Native `ReactExoplayerView` / `TextureView` measures `0` height on Fire TV, causing black video while audio plays | **Fixed** | 2026-04-20 | Architecture rewrite (2026-04-20) — collapsed player to one state-driven route, `<Video>` renders as plain flex-1 child; absolute→absolute nesting eliminated |
| MB-26 | Mobile | High | Fullscreen route renders controls only and depends on underlying persistent player; collapsed host leaves black fullscreen playback | **Fixed** | 2026-04-20 | Deleted `FullscreenPlayer` route; HomeShell renders `<PlayerSurface />` + `<ControlsOverlay />` directly when `isFullscreen && track` (2026-04-20) |
| MB-27 | Mobile | High | `transparentModal` fullscreen overlay plus persistent player underneath makes video surface layout/composition fragile on TV | **Fixed** | 2026-04-20 | Retired `transparentModal` — controls are in-tree sibling `<View>` over `<Video>`, same Window layer (2026-04-20) |
| MB-28 | Mobile | High | `react-native-video` `viewType={TEXTURE}` is ineffective because local Android `updateSurfaceView(viewType)` is a no-op | **Fixed** | 2026-04-20 | No longer relevant — reverted to stock Media3 SurfaceView (Google-recommended for Android TV) after collapsing the architecture (2026-04-20) |
| MB-29 | Mobile | Medium | Patched `exo_player_view_texture.xml` uses invalid Android XML namespace `res-android` instead of `res/android` | **Fixed** | 2026-04-20 | Patch retired entirely — `patches/react-native-video@6.19.1.patch` deleted, `patchedDependencies` removed from root package.json (2026-04-20) |
