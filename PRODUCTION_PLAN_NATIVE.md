...
**Verification status.** ~~The iOS sources have never been compiled~~ — **answered by the first Mac compile, 2026-08-31 (MK.iOS.0-pre.6 below).** The predicted failure mode played out exactly as written: a compile error on first Mac run, not silent bad ciphertext. `CCCryptorGCMOneshot*` is **not** bound (SPI-only header), one `CValuesRef` typing was wrong (`CCKeyDerivationPBKDF` binds `const char *password` as a Kotlin `String?`, not a pointer), and `CC_SHA256_DIGEST_LENGTH` binds fine. One extra rot item the list didn't predict: Kotlin/Native rejects `,` inside backticked test names (`SourceCategoryTreeBuilderTest`), which no JVM run could surface.
...
### MK.iOS.0-pre.6 — first Mac compile: the GCM binding answer (shipped 2026-08-31)

The ios-compile gate went red on its first real run (2026-08-19) for exactly the reason .2 flagged as unverified: **`CCCryptorGCMOneshotEncrypt`/`Decrypt` are not bound.** Every CommonCrypto GCM entry point — oneshot pair and legacy streaming set alike — lives in `CommonCryptoSPI.h`, a private header Kotlin/Native's platform klibs are not generated from. A custom cinterop over private SPI would link but is an App Store rejection vector; CryptoKit has the right API but is Swift-only and unreachable from Kotlin/Native. Resolution: **assemble GCM locally over the public one-shot AES primitive.**

| # | Change | Notes |
|---|---|---|
| .6a | [`AesGcm.ios.kt`](packages/shared/src/iosMain/kotlin/com/yancotv/shared/backup/AesGcm.ios.kt) — NIST SP 800-38D GCM over public `CCCrypt` AES-ECB | CTR keystream (all counter blocks in one `CCCrypt` call) + bit-serial branch-free GF(2^128) GHASH. Tag recomputed and compared constant-time BEFORE any plaintext is returned. Wire format unchanged — `iv(12) ‖ ciphertext ‖ tag(16)`, so existing Android backups restore as-is |
| .6b | `BackupCipher.ios.kt` — GCM calls swapped to `AesGcm.seal`/`open`; PBKDF2 typing fixed | Second predicted nudge: the binding maps `const char *password` to Kotlin `String?` (interop marshals UTF-8 itself) — the hand-built `CPointer` was rejected. `withPtr` helper deleted: with GCM gone its empty-array path had no reachable caller left |
| .6c | Two published McGrew/Viega AES-256-GCM vectors added to [`BackupCipherParityTest`](packages/shared/src/commonTest/kotlin/com/yancotv/shared/backup/BackupCipherParityTest.kt) (TC13 empty-plaintext, TC14 one-block) | Third-party ground truth for the cipher itself, on every target — pins the local GHASH to the standard, not merely to androidMain. Routed through `decryptBytes` (encrypt draws a random IV) |
| .6d | `SourceCategoryTreeBuilderTest` — `,` removed from a backticked test name | Kotlin/Native rejects `,` in symbol names; JVM doesn't. Third rot item, invisible to every non-Mac build |
