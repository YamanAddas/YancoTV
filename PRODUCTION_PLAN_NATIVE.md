...
6. **MK.24 — Audit follow-ups + heap-pressure bug** — planned 2026-04-28 (red-teamed revision). Sprints H + E shipped 2026-04-28. ✅ Sprint H: MB-225/226/227/228 filed. ✅ Sprint E shipped (4 commits, `d390eff` → `ae56b4d`). **Sprint I refocused 2026-04-28 from recording-subsystem to heap-pressure root cause:** I.1 device capture immediately revealed YancoTV process pinned at 376/384 MB heap with 101 consecutive `GC freed 0` events, threads blocked 12+ s on `WaitForGcToComplete`, recording rows missing because SQLDelight INSERTs can't allocate. Filed **MB-230 Critical** (heap exhaustion) — MB-229 demoted to "blocked on MB-230, retest after." Sprint I now hunts the retention chain via dumpsys meminfo per-screen + hprof dominator analysis. Four sprints remain: **I (heap pressure / OOM, ~3–8 h — in progress)**, F (motion polish, 1–2 h), G.1 (per-migration tests, 4–6 h), G.2 (corruption-recovery, 3–6 h). Strict one-per-session for I + G.1 + G.2; soft bundle allowed for F. See full plan in MK.24 section below.
...
| 24.I.4 | **Get a complete heap dump while still responsive.** As soon as I.3 identifies the worst screen, navigate to it, wait for the heap to be loaded but BEFORE the GC thrash starts. Then `adb shell am dumpheap <pid> /data/local/tmp/yancotv-heap.hprof`, `adb pull` it, run `hprof-conv` (Android SDK) to convert to standard hprof format, open in Android Studio Profiler OR run shark-cli for dominator analysis. **Find the retained-size champion.** | planned · gated on I.3 | hprof captured + dominator tree analyzed + top 5 retained-size objects identified. |
| 24.I.5 | **Map dominator to code.** Trace the retained chain back to a YancoTV code path: ViewModel field, Compose remember, Coil cache, Sentry breadcrumb buffer, ktor cache, EPG repository snapshot, etc. The dominator tree shows the GC-root → leak path; map that to source files. | planned · gated on I.4 | Specific file/line identified as the retention root. Hypothesis written into this plan as 24.I.5 body. |
| 24.I.6 | **Scope the fix.** Could be: drop a cache on `onTrimMemory`, paginate a list that's currently a `List<Foo>(N=200000)`, switch from `LiveData<List<X>>`-style snapshot to a `Flow<PagingData<X>>`-style streaming, fix a leaked Activity/ViewModel reference, etc. **Cost depends entirely on what I.5 finds.** No code is written before this slice's writeup is committed. | planned · gated on I.5 | Fix-scope writeup committed. Honest cost estimate. |
| 24.I.7 | **Ship fix + regression instrumentation.** Code change. Add a heap-watermark probe to `dumpsys meminfo`-style breadcrumbs in Sentry on `onTrimMemory(LEVEL_RUNNING_CRITICAL)` so future heap-pressure events surface even if they don't crash. Hands-on Fire TV verify: walk the same screens from I.3, confirm Java Heap doesn't climb past the new ceiling. Then attempt a scheduled recording — if it works end-to-end, MB-229 transitively closes too. | planned · gated on I.6 | Commit shipped + hands-on verification log + MB-230 + MB-229 status flips. |

**Estimate (honest, scope-corrected):** I.2 + I.3 = ~30–60 min (just navigation + meminfo). I.4 + I.5 = ~1–2 h (depends on hprof tooling responsiveness on Windows + how cleanly the dominator tree reads). I.6 scoping = 30 min. I.7 fix+test = ~1–4 h depending on what the leak is — a `clearOnTrimMemory()` hook is 30 min; switching from `selectAll().executeAsList()` to a paged `Flow` is 4–6 h. **Total: 3–8 h across 1–2 sessions.**

**Out of scope for Sprint I:** the recording-subsystem-specific work (MB-212 fix, MB-229 dual-state UI race if it persists post-fix). Those become Sprint I follow-ups only after we confirm the heap fix doesn't transitively close them.
