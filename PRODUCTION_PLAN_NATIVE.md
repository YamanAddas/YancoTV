...
6. **MK.24 — Audit follow-ups + heap-pressure bug** — planned 2026-04-28 (red-teamed revision). Sprints H + E + I + F + G.1 shipped. ✅ **Sprint G.1 shipped 2026-04-28** (`bf12d65`): 5 per-hop migration isolation tests for 3.sqm…7.sqm in `MigrationTest.kt` (582 LOC). Load-bearing test for 4.sqm catches "first row only" backfill regressions via 2-favorite fixture. ~1.5 h actual (well under 4–6 h estimate, fixture patterns reusable from v9→v10). Closes MB-227. ✅ **Sprint F closed 2026-04-28 by audit** — F.1 verified non-bug, F.2 found zero items, MK.22.A/B already shipped every relevant motion fix. ✅ **Sprint I shipped 2026-04-28** (2 commits, `f9d966f` + `a71edfe`): F1 `onTrimMemory` hook (verified -3 MB Java reclaim, 6× improvement), F2 shared OkHttpClient, F3 heap-watermark Sentry probe. 6 follow-up items tracked as 24.I.X.1–6. **One sprint remains: G.2** (DB corruption-recovery integration test, 3–6 h). Strict one-per-session.
...
Sprints H + E + I + F + G.1 complete. Remaining sequence:

1. ✅ Sprint H + E + I + F + G.1 shipped 2026-04-28 in order.
2. **Next session → Sprint G.2** (DB corruption-recovery integration test, scope depends on read-the-file gate, ~3–6 h).
