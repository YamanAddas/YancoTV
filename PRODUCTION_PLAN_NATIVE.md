...
| 5.2 | **Sideload auto-update check** — split into 3 sub-slices. **5.2.1 ✅ shipped 2026-04-28** (`2202460`): `UpdateChecker` pure-logic poller in `:shared/commonMain` + 11 table-driven tests. Polls a custom `update.json` (chosen over GitHub Releases API to avoid coupling update logic to tag-naming conventions). **5.2.2 pending:** WorkManager periodic worker (24h cadence) + reactive `StateFlow<UpdateInfo?>` + Settings → About toggle (opt-in, default on) + `BuildConfig.UPDATE_ENDPOINT` wiring from `local.properties`. **5.2.3 pending:** prompt UI + APK download with progress + `ACTION_INSTALL_PACKAGE` permission flow. On Android 8+ the user is sent to system "install unknown apps" screen for YancoTV the first time; document this in the prompt copy. | new |
