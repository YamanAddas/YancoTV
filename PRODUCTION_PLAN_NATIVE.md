...
6. **MK.24 — Audit follow-ups + heap-pressure bug** — planned 2026-04-28 (red-teamed revision). Sprints H + E + I shipped. ✅ **Sprint I shipped 2026-04-28** (2 commits, `f9d966f` + `a71edfe`): F1 `onTrimMemory` hook (verified -3 MB Java reclaim on `RUNNING_CRITICAL` vs pre-fix -0.5 MB, 6× improvement), F2 shared OkHttpClient between Coil + AndroidEpgImporter (eliminates duplicate connection pool / dispatcher / DNS / TLS), F3 heap-watermark Sentry probe (60s poll, WATCH at >75%, ALERT at >90% with `captureMessage`). 6 follow-up items tracked as 24.I.X.1–6 (high-heap hprof, RGB_565 logos, EPG retention deep-dive, MB-229 retest, Ktor pool audit, Compose retention audit) — pick up if F1+F2+F3 don't close MB-230 over a soak window. **MB-229 retest deferred** to user's normal use; F3 will surface if heap pressure recurs. Three sprints remain: F (motion polish, 1–2 h), G.1 (per-migration tests, 4–6 h), G.2 (corruption-recovery, 3–6 h). Strict one-per-session for G.1 + G.2; soft bundle allowed for F. See full plan in MK.24 section below.
