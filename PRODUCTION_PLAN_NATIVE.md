...
6. **MK.24 — Audit follow-ups + heap-pressure bug** — planned 2026-04-28 (red-teamed revision). Sprints H + E shipped. **Sprint I I.1–I.6 done 2026-04-28**: device capture confirmed runaway GC at 376/384 MB; investigation ruled out EPG sync + DB paging + background timers + single-screen retention; confirmed missing `onTrimMemory` hook + Coil double-OkHttp. **I.7 in flight: 3 fixes (F1 `onTrimMemory` hook, F2 shared OkHttp, F3 heap-watermark Sentry probe).** 6 follow-up items tracked as 24.I.X.1–6 (high-heap hprof, RGB_565 logos, EPG retention deep-dive, MB-229 retest, Ktor pool audit, Compose retention audit) — pick up if F1+F2+F3 don't close it. Three sprints remain after I.7: F (motion polish, 1–2 h), G.1 (per-migration tests, 4–6 h), G.2 (corruption-recovery, 3–6 h). Strict one-per-session for I + G.1 + G.2; soft bundle allowed for F. See full plan in MK.24 section below.
...
| 24.I.7 | **Ship F1 + F2 + F3.** Three separate commits per the one-task-per-commit rule. Hands-on Fire TV verify: install, walk the same screens from I.3, send `am send-trim-memory RUNNING_CRITICAL`, confirm Java + Native + Graphics all DROP this time (vs the earlier -0.5 MB no-op result). Soak-test by leaving the app open for 1+ hours and observing whether heap stays bounded. Recording attempt is the regression check — if it works end-to-end with a single coherent UI state, MB-229 transitively closes. | planned | 3 commits + Fire TV verification + heap-trim test + recording smoke test. MB-230 status update. MB-229 retest result documented. |
