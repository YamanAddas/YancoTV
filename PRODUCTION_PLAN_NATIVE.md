...
6. **MK.24 — Audit follow-ups + heap-pressure bug** — planned 2026-04-28 (red-teamed revision). Sprints H + E + I + F shipped (or closed-no-op). ✅ **Sprint F closed 2026-04-28 by audit** — F.1 (`CategoryRail` debounce) verified non-bug after reading file + parents (callers pass stable `MutableState` writes, fresh-lambda-per-composition is harmless); F.2 motion-polish backlog audit found zero items (MK.22.A/B already shipped every relevant fix). Zero commits — read-the-file gate working as intended, saved 1–2 h of speculative wrapping. ✅ **Sprint I shipped 2026-04-28** (2 commits, `f9d966f` + `a71edfe`): F1 `onTrimMemory` hook (verified -3 MB Java reclaim on `RUNNING_CRITICAL` vs pre-fix -0.5 MB, 6× improvement), F2 shared OkHttpClient between Coil + AndroidEpgImporter, F3 heap-watermark Sentry probe. 6 follow-up items tracked as 24.I.X.1–6. Two sprints remain: G.1 (per-migration tests, 4–6 h), G.2 (corruption-recovery, 3–6 h). Strict one-per-session for G.1 + G.2. See full plan in MK.24 section below.
...
Sprints H + E + I + F complete. Remaining sequence:

1. ✅ Sprint H + E + I + F shipped 2026-04-28 in order.
2. **Next session → Sprint G.1** (per-migration tests, test-only, ~4–6 h).
3. **Following session → Sprint G.2** (corruption-recovery, scope depends on read-the-file gate, ~3–6 h).
